function addTx(action,detail,amount,amtClass,status){
  ...
  DOM.ledgerBody.prepend(row);
    if(S.role === 'customer' && typeof buildFullSnapshot === 'function' && typeof publishLiveSnapshot === 'function'){
    publishLiveSnapshot(buildFullSnapshot({ customerId: S.customerId, heartbeatAt: Date.now() }));
  }
}
