...
- Return ONLY the JSON object, no other text.`;
  ...
  function buildRequest(cfg, userText) {
    ...
    switch (cfg.provider) {
      ...
      case 'gemini': {
        ...
        return {
          url: cfg.endpoint || `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${cfg.apiKey}`,
  ...
  function _patchProcessInput() {
    ...
    async function processInputWithAI(text) {
      ...
      let aiResult = null;
      try { aiResult = await aiClassify(t); } catch (_) { /* fallback below */ }
