...
function getVisualLogType(type){
  return type === 'supervisor' ? 'aria' : type;
}

function addLog(type,who,msg,params){
  const el = DOM.logStream;
  const now=new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',second:'2-digit'});
  const div=document.createElement('div'); div.className='entry '+getVisualLogType(type);
...
function renderLog(){
  ...
  S.logEntries.forEach(function(e){
    ...
    div.className = 'entry ' + getVisualLogType(e.type);
